//===-- MiniATAsmPrinter.cpp - XCore LLVM assembly writer ------------------===//
//
...
namespace {
  class MiniATAsmPrinter : public AsmPrinter {
  ...
  public:
    ...
    void printInlineJT32(const MachineInstr *MI, int opNum, raw_ostream &O) {
      printInlineJT(MI, opNum, O, ".jmptable");
    }
...
MiniATTargetStreamer &MiniATAsmPrinter::getTargetStreamer() {
  return static_cast<MiniATTargetStreamer&>(*OutStreamer.getTargetStreamer());
}

void MiniATAsmPrinter::emitArrayBound(MCSymbol *Sym, const GlobalVariable *GV) {
...
void MiniATAsmPrinter::EmitGlobalVariable(const GlobalVariable *GV) {
  ...
  case GlobalValue::CommonLinkage:
    report_fatal_error("Linkage is not supported by this target!");
...
void MiniATAsmPrinter::printOperand(const MachineInstr *MI, int opNum,
  ...
  case MachineOperand::MO_Register:
    O << MiniATInstPrinter::getRegisterName(MO.getReg());
    break;
...
///
bool MiniATAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNo,
                                      unsigned AsmVariant,const char *ExtraCode,
                                      raw_ostream &O) {
...
void MiniATAsmPrinter::EmitInstruction(const MachineInstr *MI) {
  ...
  switch (MI->getOpcode()) {
  case MiniAT::DBG_VALUE:
    llvm_unreachable("Should be handled target independently");
  /*case MiniAT::ADD_2rus:
...
extern "C" void LLVMInitializeMiniATAsmPrinter() { 
  RegisterAsmPrinter<MiniATAsmPrinter> X(MiniATTargetMachine);
}
