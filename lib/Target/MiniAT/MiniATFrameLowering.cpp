//===-- MiniATFrameLowering.cpp - Frame info for MiniAT Target --------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains MiniAT frame information that doesn't fit anywhere else
// cleanly...
//
//===----------------------------------------------------------------------===//
...
static const unsigned FramePtr = MiniAT::r254;
...
static void IfNeededExtSP(MachineBasicBlock &MBB,
  ...
  while (OffsetFromTop > Adjusted) {
    ...
    int Opcode = isImmU6(OpImm) ? MiniAT::EXTSP_u6 : MiniAT::EXTSP_lu6;
...
static void IfNeededLDAWSP(MachineBasicBlock &MBB,
  ...
  while (OffsetFromTop < RemainingAdj - MaxImmU16) {
    ...
    int Opcode = isImmU6(OpImm) ? MiniAT::LDAWSP_ru6 : MiniAT::LDAWSP_lru6;
    BuildMI(MBB, MBBI, dl, TII.get(Opcode), MiniAT::SP).addImm(OpImm);
    RemainingAdj -= OpImm;
  }
}
...
static void GetSpillList(SmallVectorImpl<StackSlotInfo> &SpillList,
                         MachineFrameInfo *MFI, MiniATFunctionInfo *XFI,
...
RestoreSpillList(MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
  ...
  for (unsigned i = 0, e = SpillList.size(); i != e; ++i) {
    ...
    int Opcode = isImmU6(Offset) ? MiniAT::LDWSP_ru6 : MiniAT::LDWSP_lru6;
...
MiniATFrameLowering::MiniATFrameLowering(const MiniATSubtarget &sti)
  : TargetFrameLowering(TargetFrameLowering::StackGrowsDown, 4, 0) {
  // Do nothing
}

bool MiniATFrameLowering::hasFP(const MachineFunction &MF) const {
  return MF.getTarget().Options.DisableFramePointerElim(MF) ||
         MF.getFrameInfo()->hasVarSizedObjects();
}

void MiniATFrameLowering::emitPrologue(MachineFunction &MF) const {
  ...
  if (PAL.hasAttrSomewhere(Attribute::Nest))
    BuildMI(MBB, MBBI, dl, TII.get(MiniAT::LDWSP_ru6), MiniAT::R11).addImm(0);
  ...
  if (UseENTSP)
    saveLR = false;
  bool FP = hasFP(MF);
  bool emitFrameMoves = MiniATRegisterInfo::needsFrameMoves(MF);

  if (UseENTSP) {
    // Allocate space on the stack at the same time as saving LR.
    Adjusted = (FrameSize > MaxImmU16) ? MaxImmU16 : FrameSize;
    int Opcode = isImmU6(Adjusted) ? MiniAT::ENTSP_u6 : MiniAT::ENTSP_lu6;
    MBB.addLiveIn(MiniAT::LR);
    MachineInstrBuilder MIB = BuildMI(MBB, MBBI, dl, TII.get(Opcode));
    MIB.addImm(Adjusted);
    MIB->addRegisterKilled(MiniAT::LR, MF.getTarget().getRegisterInfo(), true);
    if (emitFrameMoves) {
      EmitDefCfaOffset(MBB, MBBI, dl, TII, MMI, Adjusted*4);
      unsigned DRegNum = MRI->getDwarfRegNum(MiniAT::LR, true);
  ...
  for (unsigned i = 0, e = SpillList.size(); i != e; ++i) {
    ...
    int Opcode = isImmU6(Offset) ? MiniAT::STWSP_ru6 : MiniAT::STWSP_lru6;
  ...
  if (FP) {
    ...
    BuildMI(MBB, MBBI, dl, TII.get(MiniAT::LDAWSP_ru6), FramePtr).addImm(0);
    if (emitFrameMoves)
...
void MiniATFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (RetOpcode == MiniAT::EH_RETURN) {
    ...
    BuildMI(MBB, MBBI, dl, TII.get(MiniAT::SETSP_1r)).addReg(EhStackReg);
    BuildMI(MBB, MBBI, dl, TII.get(MiniAT::BAU_1r)).addReg(EhHandlerReg);
  ...
  if (FP) // Restore the stack pointer.
    BuildMI(MBB, MBBI, dl, TII.get(MiniAT::SETSP_1r)).addReg(FramePtr);
  ...
  if (RemainingAdj) {
    ...
    if (UseRETSP) {
      ...
      assert(RetOpcode == MiniAT::RETSP_u6
             || RetOpcode == MiniAT::RETSP_lu6);
      int Opcode = isImmU6(RemainingAdj) ? MiniAT::RETSP_u6 : MiniAT::RETSP_lu6;
      MachineInstrBuilder MIB = BuildMI(MBB, MBBI, dl, TII.get(Opcode))
                                  .addImm(RemainingAdj);
      ...
      int Opcode = isImmU6(RemainingAdj) ? MiniAT::LDAWSP_ru6 :
                                           MiniAT::LDAWSP_lru6;
      BuildMI(MBB, MBBI, dl, TII.get(Opcode), MiniAT::SP).addImm(RemainingAdj);
      // Don't erase the return instruction.
    }
  } // else Don't erase the return instruction.
}

bool MiniATFrameLowering::
spillCalleeSavedRegisters(MachineBasicBlock &MBB,
  ...
  for (std::vector<CalleeSavedInfo>::const_iterator it = CSI.begin();
    ...
    assert(Reg != MiniAT::LR && !(Reg == MiniAT::R10 && hasFP(*MF)) &&
...
eliminateCallFramePseudoInstr(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      MachineInstr *New;
      if (Old->getOpcode() == MiniAT::ADJCALLSTACKDOWN) {
        int Opcode = isU6 ? MiniAT::EXTSP_u6 : MiniAT::EXTSP_lu6;
        New=BuildMI(MF, Old->getDebugLoc(), TII.get(Opcode))
          .addImm(Amount);
      } else {
        assert(Old->getOpcode() == MiniAT::ADJCALLSTACKUP);
        int Opcode = isU6 ? MiniAT::LDAWSP_ru6 : MiniAT::LDAWSP_lru6;
        New=BuildMI(MF, Old->getDebugLoc(), TII.get(Opcode), MiniAT::SP)
          .addImm(Amount);
      }
...
processFunctionBeforeCalleeSavedScan(MachineFunction &MF,
  ...
  bool LRUsed = MF.getRegInfo().isPhysRegUsed(MiniAT::LR);
  ...
  if (LRUsed) {
    ...
    MF.getRegInfo().setPhysRegUnused(MiniAT::LR);
...
processFunctionBeforeFrameFinalized(MachineFunction &MF,
  ...
  MachineFrameInfo *MFI = MF.getFrameInfo();
  const TargetRegisterClass *RC = &MiniAT::GRRegsRegClass;
  MiniATFunctionInfo *XFI = MF.getInfo<MiniATFunctionInfo>();
